...
import { formatCombination, normalizeKeyName } from "../../src/cli/config";
...
describe("Hotkey Formatting", () => {
	...
	test("should format modifier combinations", () => {
		expect(formatCombination(new Set(["LEFT CONTROL", "SPACE"]))).toBe(
			"Ctrl+Space",
		);
		expect(formatCombination(new Set(["LEFT CONTROL", "LEFT ALT", "A"]))).toBe(
			"Ctrl+Alt+A",
		);
		expect(
			formatCombination(new Set(["RIGHT CONTROL", "LEFT SHIFT", "DELETE"])),
	...
	test("should handle node-global-key-listener specific names", () => {
		expect(formatCombination(new Set(["LEFT CONTROL", "RIGHT CONTROL"]))).toBe(
			"Ctrl",
		);
		expect(
			formatCombination(new Set(["LEFT CONTROL", "RIGHT CONTROL", "A"])),
		).toBe("Ctrl+A");
	});
});
