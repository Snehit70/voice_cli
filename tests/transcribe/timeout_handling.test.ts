...
const mocks = vi.hoisted(() => ({
  withRetry: vi.fn(async (fn: any, opts: any) => await fn()),
  groqCreate: vi.fn(),
...
describe("Timeout Handling", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mocks.withRetry.mockImplementation(async (fn: any, opts: any) => await fn());
  });

  describe("GroqClient", () => {
    it("should throw 'Groq: Request timed out' when retry times out", async () => {
      mocks.withRetry.mockImplementation(async (fn: any, opts: any) => {
