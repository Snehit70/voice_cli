	...
	test("should fallback to Nova-2 on failure", async () => {
		...
		for (let i = 0; i < TOTAL_ATTEMPTS_BEFORE_FALLBACK; i++) {
			mockTranscribeFile.mockImplementationOnce(() =>
				Promise.resolve({ error: { message: `Error ${i}` } } as any),
			);
		}
		...
		expect(mockTranscribeFile).toHaveBeenCalledTimes(
			TOTAL_ATTEMPTS_BEFORE_FALLBACK + 1,
		);

		expect((mockTranscribeFile.mock.calls as any)[0][1].model).toBe("nova-3");
		expect(
			(mockTranscribeFile.mock.calls as any)[
				TOTAL_ATTEMPTS_BEFORE_FALLBACK - 1
			][1].model,
		).toBe("nova-3");

		expect(
			(mockTranscribeFile.mock.calls as any)[TOTAL_ATTEMPTS_BEFORE_FALLBACK][1]
				.model,
		).toBe("nova-2");
	});
