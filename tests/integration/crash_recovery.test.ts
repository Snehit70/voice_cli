...
describe("Daemon Crash Recovery Integration", () => {
  ...
  it("should auto-restart after a crash", async () => {
    ...
    expect(newPid, "Daemon should have restarted with a new PID").toBeGreaterThan(0);
  ...
  it("should stop after exceeding max restarts (3 crashes)", async () => {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      expect(restarted, `Daemon should have restarted after crash ${i + 1}`).toBe(true);
    }

    process.kill(currentPid, "SIGKILL");

    await new Promise(resolve => setTimeout(resolve, 5000));
    expect(existsSync(pidFile), "PID file should not exist after exceeding max restarts").toBe(false);
