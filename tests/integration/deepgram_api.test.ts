...
describe("Deepgram API Integration", () => {
	...
	const isRealKey =
		apiKey &&
		(apiKey.length === 32 || apiKey.length === 40 || apiKey.includes("-"));

	it("should connect to Deepgram API", async () => {
		if (!isRealKey) {
			console.warn(
				"Skipping Deepgram Integration test: No valid API Key found in environment",
			);
			return;
		}
		try {
			...
			if (error.message.includes("Invalid API Key")) {
				console.warn(
					"Skipping Deepgram Integration test: Invalid API Key found in environment",
				);
				return;
			}
			throw error;
