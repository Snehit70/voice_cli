...
function displayItems(items: HistoryItem[]): void {
	items.forEach((item) => {
		...
		console.log(
			`${colors.bold("Time:")}    ${new Date(item.timestamp).toLocaleString()}`,
		);
		console.log(
			`${colors.bold("Engine:")}  ${item.engine} (${item.processingTime}ms)`,
		);
...
async function paginateHistory(
	...
	const showNextBatch = async () => {
		...
		const toShow = Math.min(
			remaining,
			displayedCount === 0 ? initialCount : 20,
		);

		if (toShow <= 0) return;

		const batch = reversedHistory.slice(
			displayedCount,
...
historyCommand
	...
	.action(async (options) => {
		...
		console.log(
			colors.bold(colors.cyan(`Last ${recent.length} transcription(s):`)),
		);
		displayItems(recent);
		console.log(colors.dim("------------------------------------------------"));
	});
...
historyCommand
	...
	.action(async (keyword, options) => {
		...
		console.log(
			colors.bold(
				colors.cyan(`Found ${results.length} matching transcription(s):`),
			),
		);
