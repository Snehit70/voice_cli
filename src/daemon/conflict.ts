...
function parseHotkey(hotkey: string): { keysym: string, modifiers: number } | null {
  const parts = hotkey.toUpperCase().split("+").map(p => p.trim());
  ...
  for (let i = 0; i < parts.length; i++) {
    ...
    if (i < parts.length - 1 && Object.prototype.hasOwnProperty.call(MODIFIERS, part)) {
       modifiers |= MODIFIERS[part]!;
       continue;
    }

    if (Object.prototype.hasOwnProperty.call(KEY_MAP, part)) {
...
export async function checkHotkeyConflict(hotkey: string): Promise<boolean> {
  ...
  try {
    ...
    const { stdout } = await execAsync(`python3 "${scriptPath}" "${keysym}" ${modifiers}`);
    
    const output = stdout.trim();
    if (output === "CONFLICT") {
      logger.warn(`Hotkey conflict detected: ${hotkey} is already in use by another application.`);
      notify("Hotkey Conflict", `The hotkey '${hotkey}' is already in use. It may not work correctly.`, "warning");
      return true;
    }
    
    return false;
  } catch (error) {
    logger.debug({ error }, "Failed to run hotkey conflict detection (likely harmless)");
    return false;
  } finally {
    try {
